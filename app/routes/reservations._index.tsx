    ...
    return <>
        ...
        <ul key="reservations">
            {reservations.map((r) => 
                <li key={r.id}>
                    <h2>{r.name}</h2>
                    {(r.userID === user.id || user.isAdmin) && <span className='button-tray'><Link to={`/reservation/${r.id}/edit`}><button className='edit'>&#9998;</button></Link><button className='delete' type='button' onClick={(e)=>handleDelete(r)}>Del</button></span>}
                    <p>Room: {r.roomID}</p>
                    <p>User: {r.userID}</p>
